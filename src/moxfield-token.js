...
// script in the isolated world can't read that token, so this runs in the page (MAIN) world,
// wraps fetch + XMLHttpRequest to observe the Authorization header Moxfield sets on its own API
// calls, and relays the latest value to the isolated content script via postMessage.
//
// The token is NEVER logged, stored, or sent anywhere except posted same-window (to our own
// content script) so the extension can edit the deck (remove / set quantity) the exact way
// Moxfield's UI does. It only enables actions the logged-in user could already do by hand.
(() => {
  let last = null;
  const relay = (v) => {
